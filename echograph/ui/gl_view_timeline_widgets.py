...
class GraphGLTimelineWidgetsMixin:
    ...
    def _build_timeline_panel(self) -> None:
        ...
        if existing is not None:
            ...
            has_fx_btn = isinstance(getattr(self, "_timeline_fx_instances_btn", None), QtWidgets.QPushButton)
        ...
        try:
            ...
            panel.setStyleSheet(
                "".join((
                    ...
                    "#GLTimelinePanel QPushButton#GLTimelineFXInstancesButton{padding:0px;background:transparent;border:0px;}",
                    "#GLTimelinePanel QPushButton#GLTimelineFXInstancesButton:hover{background:transparent;border:0px;}",
                    "#GLTimelinePanel QPushButton#GLTimelineFXInstancesButton:checked{background:transparent;border:0px;}",
            ...
            speed_lbl = QtWidgets.QLabel("Speed", panel)
            header.addWidget(speed_lbl, 0)

            speed_spin = QtWidgets.QDoubleSpinBox(panel)
            speed_spin.setRange(1.0, 1000.0)
            speed_spin.setDecimals(2)
            speed_spin.setSingleStep(10.0)
            speed_spin.setSuffix("%")
            speed_spin.setKeyboardTracking(False)
            speed_spin.setValue(float(getattr(self, "_timeline_owner_speed_percent", lambda *_: 100.0)()))
            speed_spin.setFixedWidth(78)
            speed_spin.setToolTip("Playback speed for the selected Scene outliner item.")
            speed_spin.valueChanged.connect(self._timeline_on_speed_changed)
            header.addWidget(speed_spin, 0)
            self._timeline_speed_spin = speed_spin
            self._timeline_refresh_speed_control()
            ...
            material_btn.setFixedSize(42, 34)
            ...
            fx_btn = QtWidgets.QPushButton(panel)
            fx_btn.setObjectName("GLTimelineFXInstancesButton")
            fx_btn.setCheckable(True)
            fx_btn.setFixedSize(42, 34)
            fx_btn.setFlat(True)
            fx_btn.toggled.connect(self._timeline_on_fx_instances_toggled)
            fx_btn.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
            fx_btn.customContextMenuRequested.connect(self._timeline_on_fx_instances_context_menu)
            header.addWidget(fx_btn, 0)
            self._timeline_fx_instances_btn = fx_btn
            self._update_timeline_fx_instances_button()
