...
def _collect_assets(node_item) -> List[Dict[str, str]]:
    ...
    for edge in in_edges:
        ...
        if kind == "volume_selector":
            vol_name = (getattr(model, "name", "") or "").strip()
            vol_path = _param_value(model, "path")
            vol_entry = None
            vol_edges = []
            try:
                vol_edges = list(scene._ordered_in_edges(src_item))
            except Exception:
                try:
                    vol_edges = list(scene._in_edges(src_item))
                except Exception:
                    vol_edges = []
            mesh_edge = False
            tex_edge = False
            unnamed_edges = 0
            for edge in vol_edges:
                name = getattr(edge, "dst_port_name", None) or getattr(edge, "dst_label", None) or getattr(edge, "dst_name", None)
                key = (name or "").strip().lower()
                if key in {"mesh", "target", "path"}:
                    mesh_edge = True
                elif key in {"texture", "tex"}:
                    tex_edge = True
                elif not key:
                    unnamed_edges += 1
            if not mesh_edge and unnamed_edges and not tex_edge:
                mesh_edge = True
            if not mesh_edge and len(vol_edges) >= 2:
                mesh_edge = True

            def _trace_item(item, depth=0, visited=None):
                if item is None or depth > 8:
                    return None, "", ""
                if visited is None:
                    visited = set()
                ...
                if kind == "switch":
                    try:
                        ...
                        try:
                            ...
                            edges = []
                    if edges:
                        return _trace_item(getattr(edges[0], "src", None), depth + 1, visited)
                path = _param_value(m, "path")
                return item, kind, path
            ...
            for edge in vol_edges:
                ...
                key = (name or "").strip().lower()
                if key in {"mesh", "target", "path"}:
                    mesh_src = getattr(edge, "src", None)
                elif key in {"texture", "tex"}:
                    tex_src = getattr(edge, "src", None)
            if mesh_src is None:
                for edge in vol_edges:
                    src_item = getattr(edge, "src", None)
                    src_model = getattr(src_item, "model", None) if src_item is not None else None
                    src_kind = (getattr(src_model, "kind", "") or "").strip().lower()
                    if src_kind and src_kind not in {"texture", "texture_pro", "texture_layer"}:
                        mesh_src = src_item
                        break
            if mesh_src is None and vol_edges:
                mesh_src = getattr(vol_edges[0], "src", None)
            if tex_src is None:
                for edge in vol_edges:
                    src_item = getattr(edge, "src", None)
                    src_model = getattr(src_item, "model", None) if src_item is not None else None
                    src_kind = (getattr(src_model, "kind", "") or "").strip().lower()
                    if src_kind in {"texture", "texture_pro", "texture_layer"}:
                        tex_src = src_item
                        break
            if vol_path:
                ...
                if ext in SUPPORTED_EXTS:
                    ...
                    if key not in seen:
                        ...
                        vol_entry = {
                            ...
                            "node": vol_name,
                            "ext": ext,
                            "visible": vol_name not in hidden,
                            "xform": None,
                            "wire_only": True,
                            "volume": True,
                        }
                        assets.append(vol_entry)

            # Resolve target mesh (mesh input)
            if mesh_src is not None:
                target_item, target_kind, target_path = _trace_item(mesh_src, 0, set())
            else:
                target_item, target_kind, target_path = _resolve_input_item(scene, src_item, {"mesh", "target"})
            target_model = getattr(target_item, "model", None) if target_item is not None else None
            owner_model = target_model or model
            owner_kind = (target_kind or "").strip().lower()
            path = target_path

            if owner_kind in ("texture", "texture_pro", "texture_layer"):
                upstream_item, upstream_kind, upstream_path = _resolve_input_item(scene, target_item)
            ...
            # Resolve texture override (texture input)
            if tex_src is not None:
                tex_item, tex_kind, _tex_path = _trace_item(tex_src, 0, set())
            else:
                tex_item, tex_kind, _tex_path = _resolve_input_item(scene, src_item, {"texture", "tex"})
            tex_model = getattr(tex_item, "model", None) if tex_item is not None else None
            texture_provider = None
            texture = ""
            tex_kind = (tex_kind or "").strip().lower()
            if tex_kind == "texture_pro" and tex_model is not None:
                try:
                    texture_provider = getattr(tex_model, "_texture_pro_provider", None)
                except Exception:
                    texture_provider = None
            elif tex_kind == "texture_layer" and tex_model is not None:
                try:
                    texture_provider = getattr(tex_model, "_texture_layer_provider", None)
                except Exception:
                    texture_provider = None
            elif tex_kind == "texture" and tex_model is not None:
                texture = _param_value(tex_model, "texture")

            target_name = (getattr(owner_model, "name", "") or "").strip()
            target_owner = target_name
            if not target_owner and path:
                try:
                    target_owner = Path(path).name
                except Exception:
                    target_owner = target_name
            has_target = bool(target_owner) or (target_item is not None) or mesh_edge
            has_tex = bool(texture_provider is not None or texture)
            if vol_entry is not None:
                if has_target:
                    vol_entry["wire_only"] = True
                else:
                    vol_entry["wire_only"] = False
                    if texture_provider is not None:
                        vol_entry["texture_provider"] = texture_provider
                    elif texture:
                        vol_entry["texture"] = texture
            if has_target and has_tex and target_owner:
                assets.append({
                    "volume_override": True,
                    "target_owner": target_owner,
                    "volume_owner": vol_name,
